...
export default function WorkflowBuilderJourney({ onBack }: Props) {
  ...
  const handleGuidePick = useCallback(
    (basePrompt: string, taskName: string) => {
      setPrompt(basePrompt);
      const draft = generateWorkflow(basePrompt);
      applyWorkflow({ ...draft, name: taskName }, 'guide');
    },
    [applyWorkflow],
  );
  ...
  return (
    <div className="flex flex-col h-full bg-canvas">
      ...
      {step === 1 ? (
        ...
        <div className="flex-1 min-h-0 grid grid-cols-[340px_minmax(0,1fr)_320px]">
          <AIAssistantPanel
            step={step}
